...
// format in KG
   ...
   $berat = round($berat / 1000,1,PHP_ROUND_HALF_UP);
    ...
    $costs = $this->cache->getCosts($this->config->get('jne_city_id'),$address['sub_district_id'],1000,"YES","JNE");
    }
		if ($costs) {
			$status = true;

                if ($this->cart->getSubTotal() < $this->config->get('jneyes_total')) {
			$status = false;
		}
		...
		$text=false;
		$query_filter=$this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "product_shipping_filtered WHERE shipping_code='jneyes.jneyes'");
		...
		$products=$this->getProductFiltered('jneyes.jneyes');
		  ...
		  $cost += $h_fee;
			...
			$quote_data = array();
      		...
      		$quote_data['jneyes'] = array(
        		...
        		'weight'         => $berat,
                'image'         => 'catalog/view/theme/default/image/shipping/jneyes.png',    
        		'etd'         => $costs['etd'],
